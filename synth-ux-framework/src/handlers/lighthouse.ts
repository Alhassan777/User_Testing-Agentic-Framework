...
export async function handleLighthouseAudit(args: Record<string, unknown>): Promise<string> {
  const url = args.url as string;
  const categories = (args.categories as string[]) || ['performance', 'accessibility', 'best-practices', 'seo'];
  ...
  try {
    ...
    const summary = {
      ...
      opportunities: Object.values(lhr.audits)
        .filter((a: unknown) => {
          const audit = a as { details?: { type: string; overallSavingsMs?: number } };
          return audit.details?.type === 'opportunity' && audit.details?.overallSavingsMs;
        })
        .slice(0, 5)
        .map((a: unknown) => {
          const audit = a as { title: string; description: string; details?: { overallSavingsMs: number } };
          return {
            title: audit.title,
