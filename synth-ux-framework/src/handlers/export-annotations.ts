...
import { FigmaAnnotationBatch, FigmaAnnotation } from '../figma/index.js';

const SEVERITY_COLORS: Record<string, { r: number; g: number; b: number; hex: string }> = {
...
function generateMarkdown(batch: FigmaAnnotationBatch, studyDir: string): string {
  const lines: string[] = [];
  ...
  for (const ann of batch.annotations) {
    const sev = ann.severity?.toLowerCase() as keyof typeof bySeverity;
  ...
  // Group by screenshot
  const byScreenshot = new Map<string, FigmaAnnotation[]>();
  for (const ann of batch.annotations) {
...
function generateJSON(batch: FigmaAnnotationBatch): object {
  return {
    meta: {
...
export async function handleExportAnnotations(
  ...
  const formats = (args.formats as string[]) || ['png', 'json', 'markdown'];
  ...
  const batch: FigmaAnnotationBatch = JSON.parse(fs.readFileSync(annotationsPath, 'utf-8'));
  const outputs: Record<string, string[]> = { png: [], json: [], markdown: [] };
  ...
  const byScreenshot = new Map<string, FigmaAnnotation[]>();
  for (const ann of batch.annotations) {
    if (!byScreenshot.has(ann.screenshot_path)) byScreenshot.set(ann.screenshot_path, []);
