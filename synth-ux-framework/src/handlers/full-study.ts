...
export async function handleRunFullUxStudy(args: Record<string, unknown>): Promise<string> {
  ...
  const personaCount = (args.persona_count as number) || 5;
  ...
  let studyVideoPath: string | null = null;
  ...
  try {
    ...
    const landingText: string = await (page as any).evaluate(
      // @ts-ignore
    ...
    let heuristicEval: HeuristicEvaluation | null = null;
    try {
      ...
      log(`Heuristic evaluation failed (non-fatal): ${(e as Error).message}`, 'PHASE_0');
    }

    // First-Click Analysis
    log('Running first-click analysis...', 'PHASE_0');
    let firstClickAnalysis: FirstClickAnalysis | null = null;
    try {
      ...
      log(`First-click analysis failed (non-fatal): ${(e as Error).message}`, 'PHASE_0');
    }
    ...
    const discoveredLinks = await page.$$eval('a[href]', (anchors, currentUrl) => {
      const links = anchors.map(a => a.getAttribute('href')).filter((href): href is string => !!href && !href.startsWith('#') && !href.startsWith('javascript:'));
      return links.map(href => { try { return new URL(href, currentUrl).href; } catch { return null; } }).filter((href): href is string => !!href);
    }, page.url());

    const baseHost = new URL(url).host;
    const uniqueSameHostLinks = Array.from(new Set(discoveredLinks)).filter(link => { try { return new URL(link).host === baseHost; } catch { return false; } }).slice(0, 6);
    ...
    for (const target of uniqueSameHostLinks) {
      try {
        ...
        log(`Screen capture skipped for ${target}: ${(e as Error).message}`, 'PHASE_0');
      }
    }

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const appPageText: string = await (page as any).evaluate(
      // @ts-ignore
      () => { const b = document.body.cloneNode(true); b.querySelectorAll('script,style,noscript').forEach(e => e.remove()); return b.innerText.substring(0, 4000); }
    );
    ...
    let lighthouseData: unknown = null;
    try {
      ...
      if (lhResult?.lhr) {
        lighthouseData = {
          scores: Object.fromEntries(Object.entries(lhResult.lhr.categories).map(([key, cat]) => [key, Math.round(((cat as { score: number }).score || 0) * 100)])),
      ...
      log(`Lighthouse failed (non-fatal): ${(e as Error).message}`, 'PHASE_1');
      lighthouseData = { error: (e as Error).message };
    }

    // Accessibility
    log('Running accessibility scan...', 'PHASE_1');
    let accessibilityData: unknown = null;
    try {
      ...
      log(`Accessibility scan failed (non-fatal): ${(e as Error).message}`, 'PHASE_1');
      accessibilityData = { error: (e as Error).message };
    }

    // App Brief
    log('Generating app brief...', 'PHASE_1');
    let appBrief: AppBrief | null = null;
    try {
      ...
      log(`App brief generation failed (non-fatal): ${(e as Error).message}`, 'PHASE_1');
    }
    ...
    const competitors: CompetitorProfile[] = await discoverCompetitors(appName, appDescription, landingText, 5);
    ...
    try {
      ...
      log(`Task derivation failed (non-fatal): ${(e as Error).message}`, 'PHASE_2');
    }
    ...
    const interviews: ReflectionInterview[] = [];
    const highlights: StudyHighlight[] = [];
    const timeMetricsList: TimeBasedMetrics[] = [];
    ...
    for (const persona of personas) {
      ...
      const pickScreenshot = (text: string) =>
        postLoginKeywords.test(text) ? afterLoginScreenshot : landingScreenshot;
    ...
    let designAudit: DesignAudit | null = null;
    try {
      ...
      log(`Design audit failed (non-fatal): ${(e as Error).message}`, 'PHASE_3');
    }
    ...
    let problemSeverity: ProblemSeverityMatrix | null = null;
    try {
      ...
      log(`Problem severity classification failed (non-fatal): ${(e as Error).message}`, 'PHASE_4');
    }

    log('Performing dual coding analysis...', 'PHASE_4');
    let dualCoding: DualCodingResult | null = null;
    try {
      ...
      log(`Dual coding failed (non-fatal): ${(e as Error).message}`, 'PHASE_4');
    }

    log('Generating segment heat map...', 'PHASE_4');
    let segmentHeatMap: SegmentHeatMap | null = null;
    try {
      if (problemSeverity) {
      ...
      log(`Segment heat map failed (non-fatal): ${(e as Error).message}`, 'PHASE_4');
    }

    log('Generating comprehensive analysis...', 'PHASE_4');
    let analysisResult: AnalysisResult | null = null;
    try {
      analysisResult = await generateAnalysis(personas, sessions, interviews, jtbd,
        problemSeverity || { personas: [], distribution: { level_4_hair_on_fire: 0, level_3_painful: 0, level_2_aware: 0, level_1_latent: 0 }, pmf_readiness: 'Unknown' },
        dualCoding || { themes: [], cohens_kappa: 0, reliability_rating: 'poor', disagreements: [], saturation_reached: false },
        designAudit, competitors);
      ...
      log(`Full analysis failed (non-fatal): ${(e as Error).message}`, 'PHASE_4');
    }
    ...
    try {
      ...
      log(`Calibration report failed (non-fatal): ${(e as Error).message}`, 'PHASE_4');
    }

    log('Generating A/B test hypotheses...', 'PHASE_4');
    let abHypotheses: ABHypotheses | null = null;
    try {
      ...
      log(`A/B hypotheses failed (non-fatal): ${(e as Error).message}`, 'PHASE_4');
    }
    ...
    let clips: Clip[] = [];
    let clipLibrary: ClipLibrary | null = null;
    try {
      ...
      log(`Clip extraction failed (non-fatal): ${(e as Error).message}`, 'PHASE_5');
    }
    ...
    try {
      ...
      log(`Weekly digest failed (non-fatal): ${(e as Error).message}`, 'PHASE_5');
    }
    ...
    try {
      ...
      log(`Figma annotation generation failed (non-fatal): ${(e as Error).message}`, 'PHASE_6');
    }
    ...
    return JSON.stringify({
      ...
      accessibility_violations: (accessibilityData as { violations?: number })?.violations, personas_tested: personas.map(p => ({ id: p.id, name: p.name, role: p.role })),
    ...
    await stateMachine.enterErrorState(error as Error);
    log(`Study failed in phase ${stateMachine.getPhase()}: ${(error as Error).message}`, 'ERROR');
    ...
    return JSON.stringify({
      ...
      error: (error as Error).message, can_retry: stateMachine.canRetry(),
