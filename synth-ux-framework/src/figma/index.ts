...
function hexFor(severity: string): string {
  return SEVERITY_HEX[severity?.toLowerCase()] ?? SEVERITY_HEX.medium;
}

function bgFor(severity: string): string {
 ...
 */
export function generateAnnotatedScreenshots(
  batch: FigmaAnnotationBatch,
  outputDir: string
): string[] {
...
export function generateFigmaPluginData(
  batch: FigmaAnnotationBatch,
  outputDir: string
): string {
  const pluginData = {
...
function groupAnnotationsByScreenshot(annotations: FigmaAnnotation[]): Record<string, FigmaAnnotation[]> {
  const grouped: Record<string, FigmaAnnotation[]> = {};
  for (const ann of annotations) {
    if (!grouped[ann.screenshot_path]) grouped[ann.screenshot_path] = [];
 ...
 */
export function renderAnnotationOutputs(
  batch: FigmaAnnotationBatch,
  outputDir: string
): { annotatedHtmlPaths: string[]; pluginDataPath: string } {
