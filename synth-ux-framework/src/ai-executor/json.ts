 ...
 */
export function parseJsonObject<T>(text: string, errMessage: string): T {
  const strategies = [
    ...
    () => {
      const jsonMatch = text.match(/\{[\s\S]*\}/);
      if (!jsonMatch) throw new Error('No JSON object found');
      return JSON.parse(sanitizeJson(jsonMatch[0])) as T;
    },
    ...
    () => {
      ...
      // Remove any BOM
      json = json.replace(/^\uFEFF/, '');
      return JSON.parse(sanitizeJson(json)) as T;
    },
 ...
 */
export function parseJsonArray<T>(text: string, errMessage: string): T[] {
  const strategies = [
    ...
    () => {
      ...
      if (objMatch && objMatch[1]) {
        return JSON.parse(sanitizeJson(objMatch[1])) as T[];
      }
 ...
 */
export function extractJsonField<T>(text: string, fieldName: string, defaultValue: T): T {
  try {
    const obj = parseJsonObject<Record<string, unknown>>(text, 'Field extraction failed');
    if (fieldName in obj) {
    ...
    if (match && match[1]) {
      try {
        ...
        if (val.startsWith('"') && val.endsWith('"')) {
          return val.slice(1, -1) as unknown as T;
        }
        if (val === 'true') return true as unknown as T;
        if (val === 'false') return false as unknown as T;
        if (!isNaN(Number(val))) return Number(val) as unknown as T;
      }
    }
    return defaultValue;
  }
