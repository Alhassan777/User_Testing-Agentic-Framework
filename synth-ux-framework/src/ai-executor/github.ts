...
export interface GitHubRepoInfo {
  ...
  // Tiered crawl results
  directoryTree: DirectoryNode[];
  tieredFiles: TieredFile[];
  // Derived from tiered files
  apiRoutes: string[];
  uiComponents: string[];
 ...
 */
const TIER1_EXACT: { path: string; category: string }[] = [
...
async function fetchFile(
  ...
  const data = await safeGet<GHContentFile>(
...
async function listDir(
  ...
  dirPath: string,
  headers: Record<string, string>
): Promise<GHContentFile[]> {
  const items = await safeGet<GHContentFile[]>(
...
function extractProcessEnvRefs(content: string): string[] {
 ...
 */
export async function fetchGitHubRepoInfo(repoUrl: string): Promise<GitHubRepoInfo | null> {
  const parsed = parseGitHubUrl(repoUrl);
  if (!parsed) return null;

  const { owner, repo } = parsed;
  const h = buildHeaders();
  const tieredFiles: TieredFile[] = [];
  ...
  try {
    // ─── Repo metadata ───────────────────────────────────────────────
    const repoData = await safeGet<GHRepoResponse>(
      `https://api.github.com/repos/${owner}/${repo}`, h
    );
    if (!repoData) throw new Error('Repository not found or not accessible');

    const languages = await safeGet<Record<string, number>>(
      `https://api.github.com/repos/${owner}/${repo}/languages`, h
    ) ?? {};
    ...
    for (const { path, category } of TIER1_EXACT) {
      ...
      if (category === 'readme') {
        ...
        try { packageJson = JSON.parse(content); } catch { /* ignore */ }
    ...
    for (const dir of TIER2_ROUTE_DIRS) {
      ...
      for (const rf of routeFiles) {
        if (seenRoutePaths.has(rf.path)) continue;
      ...
      if (apiRoutes.length >= 6) break; // enough route samples
    }
    ...
    for (const docDir of TIER2_DOC_DIRS) {
      ...
      for (const mdf of mdFiles) {
        ...
        if (content) tieredFiles.push({ tier: 2, category: 'docs', path: mdf.path, content });
        await delay(80);
      }
      break;
    }
    ...
    for (const { dir, category, limit } of TIER3_DIRS) {
      ...
      for (const sf of srcFiles) {
        ...
        if (category === 'service') uiComponents.push(compName);
        envVarKeys.push(...extractProcessEnvRefs(content));
        await delay(80);
      }
    }
 ...
 */
export async function generateAppBriefFromGitHub(repoUrl: string): Promise<GitHubAppBrief | null> {
...
Return ONLY valid JSON:
{
  ...
  try {
    ...
    return parseJsonObject<GitHubAppBrief>(text, 'Failed to parse GitHub app brief');
  } catch (error) {
