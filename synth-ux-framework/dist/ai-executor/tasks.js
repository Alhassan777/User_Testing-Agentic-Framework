...
import { fetchGitHubRepoInfo, generateAppBriefFromGitHub, isGitHubUrl } from './github.js';
export async function generatePersonas(appContext, count = 5) {
...
Return ONLY valid JSON array of personas, no explanation.`;
    ...
    const raw = parseJsonArray(text, 'Failed to parse personas from AI response');
    return raw.map(p => ({
        ...p,
...
export async function simulateSession(persona, pageContent, pageUrl, tasks) {
...
Return ONLY valid JSON, no explanation.`;
    const text = await callLLM(prompt, 4000);
    return parseJsonObject(text, 'Failed to parse session trace from AI response');
}
export async function conductInterview(persona, sessionSummary) {
...
Return ONLY valid JSON, no explanation.`;
    const text = await callLLM(prompt, 4000);
    return parseJsonObject(text, 'Failed to parse interview from AI response');
}
export async function analyzeJTBD(appDescription, landingPageContent) {
...
Return ONLY valid JSON, no explanation.`;
    ...
    return parseJsonObject(text, 'Failed to parse JTBD from AI response');
}
export async function discoverCompetitors(appName, appDescription, landingPageContent, count = 5) {
...
[
  {
    ...
    return parseJsonArray(text, 'Failed to parse competitors from AI response');
}
/**
 * Generate app brief from GitHub repository (enhanced intelligence)
 */
export async function generateAppBriefFromRepo(repoUrl) {
...
export async function generateCompetitorComparison(appName, appDescription, competitors, personas, sessions, interviews, jtbd) {
...
export async function generateInsights(appName, personas, sessions, interviews, jtbd, lighthouse, accessibility, extraContext) {
 ...
 */
export async function generateFigmaAnnotations(appName, visualCaptures, highlights) {
...
[
  {
    ...
    "element_description": "brief description of the element you identified"
            ...
            try {
                ...
                const rects = parseJsonArray(raw, 'Failed to parse vision annotation rects');
...
function textOnlyAnnotation(h, screenshotPath) {
    return {
        id: `ann_${h.id}`,
        screenshot_path: screenshotPath,
        title: h.title,
...
// ═══════════════════════════════════════════════════════════════════════════
export async function evaluateHeuristics(pageUrl, pageContent, screenshotDescriptions) {
...
Return ONLY valid JSON:
{
  ...
  "overall_score": 0-100
    ...
    return parseJsonObject(text, 'Failed to parse heuristic evaluation');
}
export async function analyzeFirstClick(pages, tasks) {
...
Return ONLY valid JSON:
{
  ...
  "navigation_clarity_score": 0-100
    ...
    return parseJsonObject(text, 'Failed to parse first-click analysis');
}
// ═══════════════════════════════════════════════════════════════════════════
// PHASE 1: Intelligence Gathering
// ═══════════════════════════════════════════════════════════════════════════
export async function generateAppBrief(url, landingPageContent, navigationLinks, screenshotDescriptions) {
...
Return ONLY valid JSON:
{
  ...
  "key_differentiators": ["...", "..."]
    ...
    const text = await callLLM(prompt, 4000);
    return parseJsonObject(text, 'Failed to parse app brief');
}
// ═══════════════════════════════════════════════════════════════════════════
// PHASE 2: Task & Severity Derivation
// ═══════════════════════════════════════════════════════════════════════════
export async function deriveTaskList(appBrief, personas, jtbd) {
...
Return ONLY valid JSON:
{
  ...
  "task_flow_recommended": ["task_001", "task_002", "task_003"]
    ...
    const text = await callLLM(prompt, 4000);
    return parseJsonObject(text, 'Failed to parse task list');
}
export async function classifyProblemSeverity(personas, sessions, interviews) {
...
Return ONLY valid JSON:
{
  ...
  "pmf_readiness": "Ready for PMF push / Need to find hair-on-fire segment / Still exploring problem space"
    ...
    const text = await callLLM(prompt, 3000);
    return parseJsonObject(text, 'Failed to parse problem severity');
}
// ═══════════════════════════════════════════════════════════════════════════
// PHASE 3: Design Evaluation
// ═══════════════════════════════════════════════════════════════════════════
export async function generateDesignAudit(pageContent, accessibilityResults, screenshotDescriptions) {
...
Return ONLY valid JSON:
{
  ...
  "improvement_priorities": [
    ...
    const text = await callLLM(prompt, 5000);
    return parseJsonObject(text, 'Failed to parse design audit');
}
// ═══════════════════════════════════════════════════════════════════════════
// PHASE 4: Analysis & Synthesis
// ═══════════════════════════════════════════════════════════════════════════
export async function performDualCoding(sessions, interviews) {
...
Return ONLY valid JSON:
{
  ...
  "saturation_reached": true/false
    ...
    const text = await callLLM(prompt, 5000);
    return parseJsonObject(text, 'Failed to parse dual coding');
}
export async function generateSegmentHeatMap(personas, problemSeverity, sessions, interviews) {
...
Return ONLY valid JSON:
{
  ...
  "best_pmf_opportunity": "Focus on X segment because..."
    ...
    const text = await callLLM(prompt, 4000);
    return parseJsonObject(text, 'Failed to parse segment heat map');
}
export async function generateAnalysis(personas, sessions, interviews, jtbd, problemSeverity, dualCoding, designAudit, competitors) {
...
Return ONLY valid JSON:
{
  ...
  "insights_summary": {
    ...
    return parseJsonObject(text, 'Failed to parse analysis');
}
export async function generateCalibrationReport(sessions, interviews, analysisResult) {
...
Return ONLY valid JSON:
{
  ...
  "validation_recommendations": [
    ...
    const text = await callLLM(prompt, 3000);
    return parseJsonObject(text, 'Failed to parse calibration report');
}
export async function generateABHypotheses(analysisResult, appBrief) {
...
Return ONLY valid JSON:
{
  ...
  "prioritized_order": ["ab_001", "ab_002", "..."]
    ...
    const text = await callLLM(prompt, 4000);
    return parseJsonObject(text, 'Failed to parse A/B hypotheses');
}
// ═══════════════════════════════════════════════════════════════════════════
// PHASE 5: Clip Extraction & Voice of User
// ═══════════════════════════════════════════════════════════════════════════
export async function extractClips(sessions, interviews, personas, videoPath) {
...
[
  {
    ...
    const text = await callLLM(prompt, 6000);
    return parseJsonArray(text, 'Failed to parse clips');
}
export async function buildClipLibrary(clips, sessions) {
    ...
    for (const clip of clips) {
        ...
        // By moment type
        if (!byMomentType[clip.clip_type])
            byMomentType[clip.clip_type] = [];
        byMomentType[clip.clip_type].push(clip.clip_id);
        // By persona
        if (!byPersona[clip.persona.id])
            byPersona[clip.persona.id] = [];
...
export async function generateWeeklyDigest(clipLibrary, analysisResult, weekLabel) {
...
Return ONLY valid JSON:
{
  ...
  "one_thing_to_fix": {
    ...
    const text = await callLLM(prompt, 3000);
    return parseJsonObject(text, 'Failed to parse weekly digest');
}
// ═══════════════════════════════════════════════════════════════════════════
// Session Enhancement: Time-Based Metrics
// ═══════════════════════════════════════════════════════════════════════════
export async function analyzeTimeMetrics(session, optimalSteps) {
    const actions = session.actions;
    // TTFMA: Time to first meaningful action (not just page load)
    const meaningfulActions = ['click', 'fill', 'submit', 'select'];
    const firstMeaningful = actions.find(a => meaningfulActions.some(ma => a.action.toLowerCase().includes(ma)));
