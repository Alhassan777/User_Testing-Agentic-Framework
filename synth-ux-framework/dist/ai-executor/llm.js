 ...
 */
export async function callLLM(prompt, maxTokens = 4000) {
    const provider = getLLMProvider();
    if (!provider) {
        throw new Error('Set GEMINI_API_KEY or ANTHROPIC_API_KEY in your environment. ' +
 ...
 */
export async function callLLMWithImage(prompt, imageBase64, maxTokens = 2000) {
    const provider = getLLMProvider();
    if (!provider) {
        throw new Error('Set GEMINI_API_KEY or ANTHROPIC_API_KEY in your environment.');
    }
    if (provider === 'gemini') {
        ...
        if (!text)
            throw new Error('Gemini returned empty response for image prompt');
        return text;
    }
    ...
    const text = response.content[0].type === 'text' ? response.content[0].text : '';
    if (!text)
        throw new Error('Anthropic returned empty response for image prompt');
    return text;
}
...
export function getActiveLLMProvider() {
    ...
    if (process.env.ANTHROPIC_API_KEY)
        return 'Anthropic Claude (ANTHROPIC_API_KEY)';
    return 'None set';
}
